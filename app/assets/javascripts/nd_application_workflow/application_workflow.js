...
function enable_nd_reminders() {
  $('#b_add_nd_reminder').click( function() {
    show_search_for_nd_reminder();
  });
  $('#b_nd_reminder_add_cancel').click( function() {
    $('#nd_reminder_entry').foundation('reveal','close');
    $('#nd_reminder_find_employee_processing').removeClass("ajax-processing");
  });
  $('#b_nd_reminder_find_employee').click( function() {
    find_nd_reminder_employees();
  });
  $('.nd_reminder_find_employees_on_return').keypress( function( event) {
    find_nd_reminder_employee_on_return( event, this);
  });
}
...
function show_search_for_nd_reminder() {
  $('#f_nd_reminder_input_last').val('');
  $('#f_nd_reminder_input_first').val('');
  $('#f_nd_reminder_input_net_id').val('');
  $('#nd_reminder_employee_not_found').hide();
  $('#nd_reminder_entry_results_table >tbody >tr').remove();
  $('#nd_reminder_entry_results').hide();
  $('#nd_reminder_entry').foundation('reveal','open');
}
...
function add_nd_workflow_user(row_clicked) {
    ...
    var display_name_spans = $('#nd_workflow_list span.nd_workflow_name');
    display_name_spans[i].innerText = first_name + " " + last_name;
    var display_netid_spans = $('#nd_workflow_list span.nd_workflow_netid');
    display_netid_spans[i].innerText = net_id;
    var input_first_names = $('#nd_workflow_list input.nd_workflow_assigned_to_first_name');
    input_first_names[i].value = first_name;
    var input_last_names = $('#nd_workflow_list input.nd_workflow_assigned_to_last_name');
    input_last_names[i].value = last_name;
    var input_net_ids = $('#nd_workflow_list input.nd_workflow_assigned_to_netid');
    input_net_ids[i].value = net_id;
    var current_user_netid = $('#nd_workflow_current_user_id');
    if (current_user_netid.length > 0 ) {
      var input_created_by_netid = $('#nd_workflow_list input.nd_workflow_created_by_netid');
      input_created_by_netid[i].value = current_user_netid[0].value;
    }
    var input_workflow_types = $('#nd_workflow_list input.nd_workflow_type');
    input_workflow_types[i].value = 'fyi';
    var input_workflow_manual = $('#nd_workflow_list input.nd_workflow_auto_or_manual');
    input_workflow_manual[i].value = 'manual';
    var input_nd_workflow_include_details_cb = $('#nd_workflow_list input.nd_workflow_email_include_detail_cb');
...
function add_nd_reminder_user(row_clicked) {
  var net_id = row_clicked.id.substring(9);
  var first_name = $(row_clicked).find('.ndwf_first_name').text();
  var last_name = $(row_clicked).find('.ndwf_last_name').text();
  $('#b_nd_reminder_insert').click();
  var nd_reminder_data_rows = $('#nd_reminder_list >div.nested-fields');
  i = nd_reminder_data_rows.length - 1;
  var display_name_spans = $('#nd_reminder_list span.nd_reminder_name');
  display_name_spans[i].innerText = first_name + " " + last_name;
  var display_netid_spans = $('#nd_reminder_list span.nd_reminder_netid');
  display_netid_spans[i].innerText = net_id;
  var input_first_names = $('#nd_reminder_list input.nd_workflow_assigned_to_first_name');
  input_first_names[i].value = first_name;
  var input_last_names = $('#nd_reminder_list input.nd_workflow_assigned_to_last_name');
  input_last_names[i].value = last_name;
  var input_net_ids = $('#nd_reminder_list input.nd_workflow_assigned_to_netid');
  input_net_ids[i].value = net_id;
  var current_user_netid = $('#nd_workflow_current_user_id');
  if (current_user_netid.length > 0 ) {
    var input_created_by_netid = $('#nd_reminder_list input.nd_workflow_created_by_netid');
    input_created_by_netid[i].value = current_user_netid[0].value;
  }
  var input_reminder_types = $('#nd_reminder_list input.nd_workflow_type');
  input_reminder_types[i].value = 'reminder';
  var input_reminder_manual = $('#nd_reminder_list input.nd_workflow_auto_or_manual');
  input_reminder_manual[i].value = 'manual';
  var input_nd_reminder_include_details_cb = $('#nd_reminder_list input.nd_workflow_email_include_detail_cb');
  if (input_nd_reminder_include_details_cb[i] != null) {
    input_nd_reminder_include_details_cb[i].checked = true;
  }
  // $('.nd_reminder_approval_cb').on('change',function() {
  //   set_workflow_type_on_approval_checkbox_change(this);
  // });
...
function find_nd_reminder_employee_on_return( e, field) {
  var key=e.keycode || e.which;
  if (key == 13) {
    find_nd_reminder_employees();
  }
...
function find_nd_reminder_employees() {
  $('#nd_reminder_employee_not_found').hide();
  $('#nd_reminder_employee_invalid_entries').hide();
  $('#nd_reminder_entry_results_list >div').remove();
  var lname = encodeURIComponent($('#f_nd_reminder_input_last').val());
  var fname = encodeURIComponent($('#f_nd_reminder_input_first').val());
  var netid = encodeURIComponent($('#f_nd_reminder_input_net_id').val());
  ...
  if (lname != "") {
    ...
    if (netid != "")  lookup_url = lookup_url + '/' + netid;
      ...
      $('#nd_reminder_employee_invalid_entries').show();
      return;
    }
  }
  //  lookup_url = lookup_url + ".json";
  $('#nd_reminder_find_employee_processing').addClass("ajax-processing");
  $.ajax({ url: lookup_url,
    contentType: "json",
    dataType: "json",
    cache: false,
    error: function(XMLHttpRequest, textStatus, errorThrown) {
      $('#nd_reminder_find_employee_processing').removeClass("ajax-processing");
      do_alert("An error has occurred while attempting to retrieve employee data.  Please confirm that all necessary web services are running. (Find nd_reminder employee: " + errorThrown + ")");      },
    success: function (data, status, xhr) {
      if (data.length == 0) {
        $('#nd_reminder_entry_results_list >div').remove();
        $('#nd_reminder_employee_not_found').show();
      }
      else {
        if (data[0].Employee == "None") {
          $('#nd_reminder_entry_results_list >div').remove();
          ...
          for (i = 0; i < data.length; i++) {
            ...
            if (data[i].employee_status == "A") {
              ...
              $('#nd_reminder_entry_results_list').append(row_string);
              employees_count += 1;
            }
          }
          if (employees_count == 0)
            ...
            $('#nd_reminder_entry_results').show();
        }
      }
      $('.ndwf_list').on('click', function() {
        add_nd_reminder_user(this);
