feature 'Workflow with only FYI' do
  ...
  it 'returns a single result for an exact match', js: true do
    ...
    expect(result.text).to match /Kingdon Barrett KBARRET8 ?Appl Dvlpmnt Professional ?29015, Customer IT Solutions/
  end
  ...
  it 'returns a list for a partial netid match', js: true do
    ...
    expect(the_result.text).to match /Kingdon Barrett KBARRET8 ?Appl Dvlpmnt Professional ?29015, Customer IT Solutions/
